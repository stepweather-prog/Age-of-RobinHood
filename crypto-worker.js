// ============================================================
//  CRYPTO-WORKER.JS - Криптографический воркер
//  X3DH + Double Ratchet + Игровые функции
// ============================================================
...
// ===== Игровые крипто-функции =====

async function encryptGameState(state, keyHex) {
    const jsonString = JSON.stringify(state);
    const encrypted = await encryptAES(jsonString, keyHex);
    const hmac = await computeHMAC(encrypted, keyHex);
    return JSON.stringify({ data: encrypted, hmac: hmac });
}

async function decryptGameState(encryptedState, keyHex) {
    try {
        const parsed = JSON.parse(encryptedState);
        const hmacValid = await verifyHMAC(parsed.data, parsed.hmac, keyHex);
        if (!hmacValid) return null;
        const decrypted = await decryptAES(parsed.data, keyHex);
...
async function generateGameToken(playerId, secretKey) {
...
async function verifyGameToken(token, publicKey) {
    try {
        const parsed = JSON.parse(token);
        const isValid = await verify(parsed.data, parsed.signature, publicKey);
    ...
    try {
        ...
        switch (action) {
            ...
            case 'dhRatchetReceive':
                ...
                break;
            case 'encryptGameState':
                result = await encryptGameState(payload.state, payload.key);
                break;
            case 'decryptGameState':
                result = await decryptGameState(payload.encrypted, payload.key);
                break;
            case 'generateGameToken':
                result = await generateGameToken(payload.playerId, payload.secretKey);
                break;
            case 'verifyGameToken':
                result = await verifyGameToken(payload.token, payload.publicKey);
                break;
            default:
